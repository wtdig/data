...
public class JobRunner extends EventHandler implements Runnable {
    ...
    public void run() {
        try {
            this.flowLogger.info("开始运行doRun");
            doRun();
        } catch (final Exception e) {
    ...
    private void doRun() {
        ...
        try {
            ...
            //TODO 向数据库修改状态之前，判断依赖逻辑。因依赖逻辑的状态变更问题无法在shell脚本中处理，变更之后，会再次变更回来
            //TODO 环境切换后续优化
            String baseUrl;
            String uatUrl = "http://mkl-datacenter.uat1.rs.com/data-c";
            String stgUrl = "https://mkl-datacenter.redstarclouds.cn/data-c";
            ...
            this.flowLogger.info("开始执行依赖逻辑的任务：" + flowId);
            //第一步：判断该任务的状态是否为失败，如果为失败，就不需要写入状态，将最终状态设置为失败
            String url = baseUrl + "/api/jobService/taskJob/checkFlowStatusByFlowId?flowId=" + flowId;
            //发送get请求
            ...
            try {
                ...
                if (response != null && response.getStatusLine().getStatusCode() == 200) {
                    this.flowLogger.info("请求成功了,flow:" + flowId);
                    //TODO 获取结果状态
                    HttpEntity entity = response.getEntity();
                    String result = EntityUtils.toString(entity);
                    String standResultString = "{\"dataMap\":true,\"code\":200,\"message\":\"success\"}";
                    boolean isError = standResultString.equals(result);
                    this.flowLogger.info("获取的结果数据为" + result + "isError为：" + isError);
                    if (!isError) {
                        //只有这一行是源码
                        writeStatus();
                    } else {
            ...
            this.flowLogger.info("结束执行依赖逻辑的任务：" + executableFlow);
        } finally {
